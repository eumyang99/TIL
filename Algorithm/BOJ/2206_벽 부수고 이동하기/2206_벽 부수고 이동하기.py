...
## [벽을 깨지 않은 거리를 담은 2차원 배열, 벽을 깬 거리를 담은 2차원 배열] -> 3차원 배열
## 정석적인 방법으로 풀어 봤음
...
memo = [[[-1]*m for _ in range(n)] for _ in range(2)]

que = deque([(0, 0, 1, False)])
memo[0][0][0] = 1
memo[1][0][0] = 1
while que:
    x, y, d, is_break = que.popleft()
    if (x, y) == (n-1, m-1):
        print(d)
        break
    for i in range(4):
        ...
        if 0 <= nx < n and 0 <= ny < m:
            ...
            if not is_break:
                if lst[nx][ny] == 0:
                    if memo[0][nx][ny] == -1:
                        memo[0][nx][ny] = d + 1
                        que.append((nx, ny, d + 1, False))
                else: # 벽을 깨고 가는 상황이기 때문에
                    if memo[1][nx][ny] == -1:
                        memo[1][nx][ny] = d + 1
                        # 벽을 깼다는 내용을 que에 담음
                        que.append((nx, ny, d + 1, True))
            
            ## 벽을 깬 상황이면
            ## 벽을 깨고 간 배열에만 기록할 수 있고 벽이 없는 곳에만 갈 수 있음
            elif is_break:
                if lst[nx][ny] == 0 and memo[1][nx][ny] == -1:
                    memo[1][nx][ny] = d + 1
                    que.append((nx, ny, d + 1, True))
else:
    print(-1)
